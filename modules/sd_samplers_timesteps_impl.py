...
@torch.no_grad()
def ddim(model: Any, x: torch.Tensor, timesteps: torch.Tensor, extra_args: Optional[Dict[str, Any]] = None, callback: Optional[Callable] = None, disable: Optional[bool] = None, eta: float = 0.0):
...
@torch.no_grad()
def ddim_cfgpp(model: Any, x: torch.Tensor, timesteps: torch.Tensor, extra_args: Optional[Dict[str, Any]] = None, callback: Optional[Callable] = None, disable: Optional[bool] = None, eta: float = 0.0):
    ...
    for i in tqdm.trange(len(timesteps) - 1, disable=disable):
        ...
        # Check if last_noise_uncond exists, if not, use e_t as a fallback
        last_noise_uncond = getattr(model, 'last_noise_uncond', e_t)
...
@torch.no_grad()
def plms(model: Any, x: torch.Tensor, timesteps: torch.Tensor, extra_args: Optional[Dict[str, Any]] = None, callback: Optional[Callable] = None, disable: Optional[bool] = None):
    alphas_cumprod = model.inner_model.inner_model.alphas_cumprod
    alphas = alphas_cumprod[timesteps]
    alphas_prev = alphas_cumprod[torch.nn.functional.pad(timesteps[:-1], pad=(1, 0))].to(float64(x))
    sqrt_one_minus_alphas = torch.sqrt(1 - alphas)
...
class UniPCCFG(uni_pc.UniPC):
    def __init__(self, cfg_model: Any, extra_args: Dict[str, Any], callback: Callable, *args, **kwargs):
        ...
        def after_update(self, x: torch.Tensor, model_x: torch.Tensor) -> None:
...
def unipc(model: Any, x: torch.Tensor, timesteps: torch.Tensor, extra_args: Optional[Dict[str, Any]] = None, callback: Optional[Callable] = None, disable: Optional[bool] = None, is_img2img: bool = False):
    try:
        alphas_cumprod = model.inner_model.inner_model.alphas_cumprod
        ...
        print(f"Error in unipc sampling: {str(e)}")
        raise
